...
if [ -d "${PLUGIN_PATH}" ] || [ "$INSTALL_SUCCESS" = true ]; then
  ...
  echo ""
  echo "  2. Run any model with thinking budget (e.g. Gemini, Claude):"
  echo "     omp --model google-antigravity/gemini-3-flash --thinking high"
  echo "     omp --model google-antigravity/claude-sonnet-4-6 --thinking high"
  echo ""
